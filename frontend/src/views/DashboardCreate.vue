...
import { createDashboards } from '@/composables/dashboard/dashboardApi.ts'
...
const saveDashboardConfig = async () => {
  const config = dashboardItems.value.map((item) => ({
    id: item.id,
    componentName: item.component.__name || item.component.name || item.name,
    props: item.props,
    data: item.data,
  }))

  await createDashboards(config)
}
</script>
